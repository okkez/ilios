...
- Add `Future#on_complete`, which handles both outcomes with a single callback: the block is always called with exactly two arguments, `(value, nil)` on success and `(nil, ExecutionError)` on failure, exactly once. There is no arity special-casing, so a lambda callback must accept both arguments. It cannot be combined with `on_success` / `on_failure` on the same future, in either direction, and `Future#await` waits for its delivery like for the other callbacks
- Add `Ilios::Cassandra::Future.all`, returning a `Future::All` aggregate that resolves once every given future completed. `Future::All#on_complete` yields the failures (an empty array when everything succeeded) exactly once, and `Future::All#await` waits for all the futures and for that callback. The aggregate registers `on_complete` on each future, so the futures must not already carry a callback, and the same future must not be given twice
